function runbetatdpilchank(filename,filesave,sizepar,mex)
% sizepar should be:
...
codegen zpilchank -args {vectorType2,vectorType1,vectorType1,vectorType1l,1,1,1,1,1,1}
    tic;
    [newmap,newbetadict,K]=zpilchank_mex(EBSD,IQ,CI,beta,5,10,3,3,1,numwanted);
    timing=toc;
    save(['results/' filesave 'P'],'newmap','newbetadict','timing');
    kappa=ones(1,K)*50;
else
    tic;
    [newmap,newbetadict,K]=zpilchank(EBSD,IQ,CI,beta,5,10,3,3,1,numwanted);
    timing=toc;
    save(['results/' filesave 'P'],'newmap','newbetadict','timing');
    kappa=ones(1,K)*50;
